...
def RndTform(img,val, boolean):
    ...
    func_directory = {1: margin, 2:noisy}
    #func_directory = {1: wavy, 2: noisy}
    ret = []    

    for i in range(len(img)):       
      img2 = np.uint8(np.copy(img[i])*255)
      if random.choice([True, False, True]):
        selec = random.choice([-1, 1])
        img2 = slant(img2, selec)
      ...
      # imr = to_mean(imr)
      # imr = wavy(imr)
      # imr = cv2.GaussianBlur(imr, (5, 5), cv2.BORDER_DEFAULT)
      # imr = np.expand_dims(imr, -1)
      ...
      # s = s+[0]
      # alpha = max(s) + 1
      # cv2.imwrite('/content/drive/MyDrive/OrigamiNet/distorted_results/'+str(alpha) + '.png', np.uint8(img2))
      ret.append(img2/255)
