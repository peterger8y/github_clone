...
def noisy(image, noise_typ='gauss', factor=1):    
    ...
    if noise_typ == "gauss":
        ...
        var = 0.1
        sigma = var ** 0.5
        gauss = np.uint8(np.random.normal(mean, sigma, (row, col, dim)) * factor)
        noisy = image + gauss
...
def aug_ED2(imgs, w, h, n_ch, tst=False):
    d = Distort3(1.0, 5, 5, 2, 4, [w, h], 1, 1)
...
def pageup(im1):
    ...
    while loc < im.shape[0]:
        ...
        im[loc - width:loc, :].fill(random.randint(15, 40))
        loc += line_spacing
